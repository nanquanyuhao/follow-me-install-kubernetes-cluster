...
b0w6e9.ncqg5iuauqdqoows   23h       2019-01-10T18:57:24+08:00   authentication,signing   kubelet-bootstrap-token   system:bootstrappers:kube-node2
j7c1yg.b84bl0erfxvn56n0   23h       2019-01-10T18:57:25+08:00   authentication,signing   kubelet-bootstrap-token   system:bootstrappers:kube-node3
vqm1lj.8507eexl17e1eyy6   23h       2019-01-10T18:57:18+08:00   authentication,signing   kubelet-bootstrap-token   system:bootstrappers:kube-node1
```
...
NAME                                             TYPE                                  DATA      AGE
attachdetach-controller-token-gdrtr              kubernetes.io/service-account-token   3         1h
bootstrap-signer-token-4bjmg                     kubernetes.io/service-account-token   3         1h
bootstrap-token-b0w6e9                           bootstrap.kubernetes.io/token         7         1m
bootstrap-token-j7c1yg                           bootstrap.kubernetes.io/token         7         1m
bootstrap-token-vqm1lj                           bootstrap.kubernetes.io/token         7         1m
certificate-controller-token-g8chr               kubernetes.io/service-account-token   3         1h
clusterrole-aggregation-controller-token-pwt2s   kubernetes.io/service-account-token   3         1h
cronjob-controller-token-dsdph                   kubernetes.io/service-account-token   3         1h
daemon-set-controller-token-zs8b7                kubernetes.io/service-account-token   3         1h
default-token-kwlcw                              kubernetes.io/service-account-token   3         1h
deployment-controller-token-ng9l7                kubernetes.io/service-account-token   3         1h
disruption-controller-token-xnf8w                kubernetes.io/service-account-token   3         1h
endpoint-controller-token-ww5cn                  kubernetes.io/service-account-token   3         1h
generic-garbage-collector-token-lstq7            kubernetes.io/service-account-token   3         1h
horizontal-pod-autoscaler-token-fwpdg            kubernetes.io/service-account-token   3         1h
job-controller-token-cncbg                       kubernetes.io/service-account-token   3         1h
namespace-controller-token-m5qxx                 kubernetes.io/service-account-token   3         1h
node-controller-token-scgnr                      kubernetes.io/service-account-token   3         1h
persistent-volume-binder-token-jqcmt             kubernetes.io/service-account-token   3         1h
pod-garbage-collector-token-k75dh                kubernetes.io/service-account-token   3         1h
pv-protection-controller-token-5tvw7             kubernetes.io/service-account-token   3         1h
pvc-protection-controller-token-qm6w5            kubernetes.io/service-account-token   3         1h
replicaset-controller-token-v9np7                kubernetes.io/service-account-token   3         1h
replication-controller-token-qcj7b               kubernetes.io/service-account-token   3         1h
resourcequota-controller-token-b2wkp             kubernetes.io/service-account-token   3         1h
service-account-controller-token-wj4pp           kubernetes.io/service-account-token   3         1h
service-controller-token-546jx                   kubernetes.io/service-account-token   3         1h
statefulset-controller-token-6ck7l               kubernetes.io/service-account-token   3         1h
token-cleaner-token-bjk68                        kubernetes.io/service-account-token   3         1h
ttl-controller-token-hj6zj                       kubernetes.io/service-account-token   3         1h
```
...
替换后的 kubelet.config.json 文件： [kubelet.config.json](./systemd/kubelet.config.json)
...
node-csr-GOytFwrgnCK5qkXKZ2ocqVkMMOq33ONI0x72Hup5-6I   27s       system:bootstrap:vqm1lj   Pending
node-csr-WA4k0CB3TakLVwbjg7Twby2FmlDMjuCx16CquT7kOWc   24s       system:bootstrap:j7c1yg   Pending
node-csr-vvZaayGHx-mrM_Jj3G4Xi9XbUyWvioTFkKKLu73HCcA   23s       system:bootstrap:b0w6e9   Pending
...
kube-node1   Ready     <none>    2m        v1.10.4
kube-node2   Ready     <none>    2m        v1.10.4
kube-node3   Ready     <none>    2m        v1.10.4
```
...
tcp        0      0 192.168.235.123:10250   0.0.0.0:*               LISTEN      4118/kubelet        
tcp        0      0 192.168.235.123:4194    0.0.0.0:*               LISTEN      4118/kubelet        
tcp        0      0 127.0.0.1:10248         0.0.0.0:*               LISTEN      4118/kubelet 
```
...
``` bash
$ curl -s --cacert /etc/kubernetes/cert/ca.pem https://192.168.235.124:10250/metrics
Unauthorized

$ curl -s --cacert /etc/kubernetes/cert/ca.pem -H "Authorization: Bearer 123456" https://192.168.235.124:10250/metrics
Unauthorized
```
...
``` bash
$ # 权限不足的证书；
$ curl -s --cacert /etc/kubernetes/cert/ca.pem --cert /etc/kubernetes/cert/kube-controller-manager.pem --key /etc/kubernetes/cert/kube-controller-manager-key.pem https://192.168.235.124:10250/metrics
Forbidden (user=system:kube-controller-manager, verb=get, resource=nodes, subresource=metrics)

$ # 使用部署 kubectl 命令行工具时创建的、具有最高权限的 admin 证书；
$ curl -s --cacert /etc/kubernetes/cert/ca.pem --cert ./admin.pem --key ./admin-key.pem https://192.168.235.124/metrics|head
...
$ curl -s --cacert /etc/kubernetes/cert/ca.pem -H "Authorization: Bearer ${TOKEN}" https://192.168.235.124:10250/metrics|head
# HELP apiserver_client_certificate_expiration_seconds Distribution of the remaining lifetime on the certificate used to authenticate a request.
# TYPE apiserver_client_certificate_expiration_seconds histogram
apiserver_client_certificate_expiration_seconds_bucket{le="0"} 0
apiserver_client_certificate_expiration_seconds_bucket{le="21600"} 0
...
浏览器访问 http://192.168.235.123:4194/containers/ 可以查看到 cadvisor 的监控页面：

![cadvisor-home](images/cadvisor-home.png)

浏览器访问 https://192.168.235.123:10250/metrics 和 https://192.168.235.123:10250/metrics/cadvisor 分别返回 kublet 和 cadvisor 的 metrics。
