...
将加密配置文件拷贝到 master 节点的 `/etc/kubernetes` 目录下（以下命令是拷贝到了全部节点下，暂猜测由于做了高可用，全部节点都需要装，所以都有）：
...
替换后的 encryption-config.yaml 文件：[encryption-config.yaml](./systemd/encryption-config.yaml)
...
替换后的 unit 文件：[kube-apiserver.service](./systemd/kube-apiserver.service)
